            ...
            <div class="col-md-12">
                <div class="panel panel-default">
                    <div class="panel-heading text-center">
                        {{ trans('User.index.header') }}
                    </div>
                    <div class="panel-body">
                        @if(auth()->user()->role === 'admin')
                            <a title="{{ trans('User.index.add_user') }}" class="btn btn-primary"
                        ...
                        <table class="table table-striped">
                            <thead>
                            <tr>
                                <th>{{ trans('User.index.no') }}</th>
                                <th>{{ trans('User.index.firstname') }}</th>
                                <th>{{ trans('User.index.surname') }}</th>
                                <th></th>
                                <th></th>
                            </tr>
                            </thead>
                            <tbody>
                                @foreach($usersPaginator as $i => $user)
                                    <tr class="clickable-row" data-href="{{ route('userprofile', ['userId' => $user->getUserId()]) }}">
                                        ...
                                        <td>
                                            <a title="{{ trans('User.index.show') }}" class="btn btn-default"
