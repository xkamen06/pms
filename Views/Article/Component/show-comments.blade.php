<h4>{{ trans('Article.show.comments') }}</h4>
    ...
    @foreach($comments as $comment)
        <tr>
            <td>
                ...
                <span class="badge">{{ trans('Article.show.added') }} {{ $comment->getAddedAt() }}</span>
                    ...
                    <div class="col-sm-10">
                        <form class="form-horizontal" action="{{ route('updatearticlecomment', ['commentId' => $comment->getCommentId(),
                            ...
                            <div class="form-group">
                                ...
                                <button class="btn btn-default" type="submit" title="{{ trans('Article.show.edit') }}">
                                    {{ trans('Article.show.edit') }}
                                </button>
                                <a href="{{ route('articlebyid', ['articleId' => $article->getArticleId()]) }}" class="btn btn-default"
                                   title="{{ trans('Article.show.cancel') }}">
                                    {{ trans('Article.show.cancel') }}
                                </a>
                            </div>
                        </form>
                    </div>
                    ...
                    @if($editCommentId === null)
                        ...
                        @if(auth()->user()->role === 'admin' || $comment->getUserId() === auth()->user()->id
                            ...
                            <a href="{{ route('deletecomment', ['commentId' => $comment->getCommentId()]) }}"
                               title="{{ trans('Article.show.delete_comment') }}" class="btn-xs btn-danger"
                               onclick="
                                       return confirm('<?php echo (trans('Article.show.are_you_sure_comment')
                                   . trans('Article.show.from_article'))?>');">
        ...
        <div class="col-sm-10">
            <form class="form-horizontal" action="{{ route('addarticlecomment', ['artcileId' => $article->getArticleId()]) }}" method="post">
                ...
                <div class="form-group">
                    ...
                    <button class="btn btn-primary" type="submit" title="{{ trans('Article.show.add_comment') }}">
