            ...
            <div class="col-md-12">
                <div class="panel panel-default">
                    <div class="panel-heading text-center">
                        {{ trans('Article.index_by_team.articles') }}
                    </div>
                    <div class="panel-body">
                        @if(auth()->user()->role === 'admin' || $team->isMember(auth()->user()->id)
                            ...
                            <a href="{{ route('addarticleform', ['teamId' => $team->getTeamId()]) }}" class="btn btn-primary"
                                title="{{ trans('Article.index_by_team.add_article') }}">
                                    ...
                                    <td>
                                        <h4>
                                            ...
                                            @if($article->getType() === 'requirement')
                                                <span class="label label-warning">{{ trans('Article.index_by_team.requirement') }}</span>
                                            @elseif($article->getType() === 'info')
                                                <span class="label label-primary">{{ trans('Article.index_by_team.info') }}</span>
                                            @else
                                                <span class="label label-danger">{{ trans('Article.index_by_team.attention') }}</span>
                                            @endif
                                        </h4>
                                        ...
                                        <span class="badge">{{ trans('Article.index_by_team.added') }} {{ $article->getAddedAt() }}</span>
                                        @if($article->getOwner())
                                        ...
                                        <div class="text-right">
                                            @if(auth()->user()->role === 'admin' || $article->getUserId() === auth()->user()->id)
                                                <a href="{{ route('editarticle', ['articleId' => $article->getArticleId()]) }}"
                                                   class="btn btn-default" title="{{ trans('Article.index_by_team.edit_article') }}">
                                            ...
                                            @if(auth()->user()->role === 'admin' || $article->getUserId() === auth()->user()->id
                                                ...
                                                <a href="{{ route('deletearticle', ['articleId' => $article->getArticleId()]) }}"
                                                   class="btn btn-danger" title="{{ trans('Article.index_by_team.delete_article') }}"
                                                   onclick="
