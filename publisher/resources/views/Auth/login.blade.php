...
@section('content')
    ...
    <div class="container">
        <div class="card card-container">
            <div class="login">
                <h1>{{ trans('pms::Auth.login.sign_in') }}</h1>
            </div>
            ...
            <form class="form-signin" method="POST" action="{{ route('login') }}">
                ...
                <div id="remember" class="checkbox">
                    <label>
                        <input type="checkbox" value="remember-me"> {{ trans('pms::Auth.login.remmember_me')}}
                    </label>
                </div>
                <button class="btn btn-lg btn-primary btn-block btn-signin" type="submit">
                    {{ trans('pms::Auth.login.sign_in') }}
                </button>
            </form><!-- /form -->
            {{--<a href="#" class="forgot-password">--}}
                 {{--{{ trans('pms::Auth.login.forget_password') }}--}}
            {{--</a>--}}
        </div><!-- /card-container -->
    </div><!-- /container -->
@endsection
